...
A complete deepfake detection workflow built around a hybrid architecture that marries the Swin-Tiny transformer for spatial feature extraction with an LSTM + attention head for temporal reasoning. The project trains on the full DeepFake Detection Challenge (DFDC) dataset, then serves the resulting model through a FastAPI + Streamlit web application.
